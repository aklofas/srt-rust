...
**Status:** early development. The `srt-sys` crate (raw FFI + encryption) is implemented; the safe API and binding crates are not yet started.
...
`srt-sys` is intended as a foundation for higher-level crates in this workspace. Most callers should wait for `srt-core`'s safe API rather than write `unsafe` against the raw bindings directly.

## Building

### Prerequisites

- Rust 1.85+ (the workspace pins 1.85 via `rust-toolchain.toml`).
...
```bash
SRT_FORCE_VENDORED=1 cargo test -p srt-sys
SRT_FORCE_VENDORED=1 cargo test -p srt-sys --no-default-features  # unencrypted variant
```
