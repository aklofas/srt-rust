...
pub unsafe extern "C" fn tst_tcp_demux_receiver_get_stream_codec_stats(
    ...
    unsafe {
        crate::transport_impls::demux_receiver_get_stream_codec_stats(
            ...
            pid,
            out,
            &format!("codec stats not available for pid 0x{pid:04x} (pid has never been observed on this tcp demux receiver)"),
        )
    }
}
...
mod tests {
    ...
    fn null_next_event_returns_invalid_config() {
        ...
        let rc =
            unsafe { tst_tcp_demux_receiver_next_event(std::ptr::null_mut(), &mut ev) };
    ...
    fn open_with_null_url_returns_null() {
        let p =
            unsafe { tst_tcp_demux_receiver_open(std::ptr::null(), std::ptr::null()) };
        assert!(p.is_null());
    }
}
