/*
 * udp_basic.c — Minimal raw UDP sender: pre-built TS packets over plain UDP.
 *
 ...
 *      -Wall -Werror \
 *      -o /tmp/udp_basic \
 *      bindings/c/tst-c/examples/c/sending/udp_basic.c -ltstrans -lpthread -ldl
 *   LD_LIBRARY_PATH=target/debug /tmp/udp_basic
 *
 * Run (unicast, default):
 *   LD_LIBRARY_PATH=target/debug /tmp/udp_basic
 *
 * Run with explicit destination (unicast):
 *   LD_LIBRARY_PATH=target/debug /tmp/udp_basic --dest 192.168.1.100:5000
 *
 * Run (multicast):
 *   LD_LIBRARY_PATH=target/debug /tmp/udp_basic --dest 239.1.2.3:5000
 *
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--dest") == 0 && i + 1 < argc) {
            ...
            fprintf(stderr, "usage: udp_basic [--dest host:port]\n"
                    "  Default dest: %s\n"
    ...
    fprintf(stderr, "[udp_basic] destination: %s\n", url);
    ...
    if (!sender) {
        fprintf(stderr, "[udp_basic] tst_udp_sender_open failed: %s\n",
                tst_get_last_error_str());
        return 2;
    }
    fprintf(stderr, "[udp_basic] sender opened; pushing %d TS packets\n",
            SEND_COUNT);
    ...
    for (int i = 0; i < SEND_COUNT; i++) {
        ...
        if (rc != 0) {
            fprintf(stderr,
                    "[udp_basic] tst_udp_sender_send_ts[%d] failed (rc=%d): %s\n",
    ...
    if (exit_code == 0) {
        fprintf(stderr,
                "[udp_basic] sent %d TS packets (%d bytes) successfully.\n",
    ...
    fprintf(stderr, "[udp_basic] sender closed.\n");
