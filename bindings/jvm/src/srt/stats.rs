//! Build the `org.tstrans.srt.SrtStats` / `ManagedTransportStats` Java records
//! from the Rust stats structs (`SocketStats` moved to the shared
//! `jutil::build_socket_stats`). All integer counters widen to `i64` (Java has
//! no unsigned types — reinterpret the bit pattern; documented in the Java
//! records). The `f64` field `mbpsEstimatedBandwidth` passes through as a JNI
//! `jdouble`.
