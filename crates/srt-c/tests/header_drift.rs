...
fn committed_header_matches_cbindgen_output() {
    ...
    let committed = PathBuf::from(manifest_dir).join("include/srtc.h");
    let expected = fs::read_to_string(&committed).expect("read committed srtc.h");
    ...
    if generated != expected {
        eprintln!(
            "Drift between crates/srt-c/include/srtc.h and cbindgen output. \
             Run `cargo build -p srt-c && cp target/debug/include/srtc.h crates/srt-c/include/srtc.h` \
             and commit the diff."
        );
