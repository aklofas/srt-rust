...
//! (`srtc_mux_config_add_program`, `_add_video_stream`, `_add_klv_stream`),
...
use srtc::muxer::{
    srtc_muxer_close, srtc_muxer_open, srtc_muxer_pull, srtc_muxer_push_klv_to,
    srtc_muxer_push_video_to,
};
...
fn two_program_muxer_pat_carries_both_programs() {
    unsafe {
        ...
        let p1 = srtc_mux_config_add_program(cfg, 1, 0x1000);
        assert_ne!(p1, SrtcProgramHandle(u32::MAX), "p1 handle must be valid");
        let v1 = srtc_mux_config_add_video_stream(cfg, p1, 0x1011, SrtcVideoCodec::H264);
        assert_ne!(v1, SRTC_INVALID_STREAM_HANDLE);
        let k1 =
            srtc_mux_config_add_klv_stream(cfg, p1, 0x1031, SrtcKlvStreamType::PrivateData, false);
        assert_ne!(k1, SRTC_INVALID_STREAM_HANDLE);

        // Program 2: PMT on 0x1100, video on 0x1111, KLV on 0x1131.
        // PIDs must be unique across programs — use a distinct range.
        let p2 = srtc_mux_config_add_program(cfg, 2, 0x1100);
        assert_ne!(p2, SrtcProgramHandle(u32::MAX), "p2 handle must be valid");
        assert_ne!(p1, p2, "program handles must be distinct");
        let v2 = srtc_mux_config_add_video_stream(cfg, p2, 0x1111, SrtcVideoCodec::H264);
        assert_ne!(v2, SRTC_INVALID_STREAM_HANDLE);
        assert_ne!(v1, v2, "cross-program video handles must be distinct");
        let k2 =
            srtc_mux_config_add_klv_stream(cfg, p2, 0x1131, SrtcKlvStreamType::PrivateData, false);
        assert_ne!(k2, SRTC_INVALID_STREAM_HANDLE);

        // ── Open muxer ────────────────────────────────────────────────────
        let mux = srtc_muxer_open(cfg);
        // Free config immediately after open — muxer owns its own copy.
        srtc_mux_config_free(cfg);
        assert!(!mux.is_null(), "srtc_muxer_open returned null");
        ...
        for tick in 0u64..30 {
            ...
            let rc1 = srtc_muxer_push_video_to(mux, v1, NAL_IDR.as_ptr(), NAL_IDR.len(), pts, true);
            assert_eq!(
                rc1, 0,
                "push video to program 1 failed at tick {tick}: rc={rc1}"
            );
            let rc2 = srtc_muxer_push_video_to(mux, v2, NAL_IDR.as_ptr(), NAL_IDR.len(), pts, true);
            assert_eq!(
                rc2, 0,
                "push video to program 2 failed at tick {tick}: rc={rc2}"
            );
            let rk1 = srtc_muxer_push_klv_to(mux, k1, KLV_BLOB.as_ptr(), KLV_BLOB.len(), pts);
            assert_eq!(
                rk1, 0,
                "push KLV to program 1 failed at tick {tick}: rc={rk1}"
            );
            let rk2 = srtc_muxer_push_klv_to(mux, k2, KLV_BLOB.as_ptr(), KLV_BLOB.len(), pts);
            assert_eq!(
                rk2, 0,
