...
// Re-export the pure-parsing error types from tst-core so existing code
// using `srt_core::error::{MuxError, DemuxError, KlvDecodeError, ...}` keeps compiling.
pub use tst_core::error::{
    DemuxError, KlvDecodeError, KlvEncodeError, KlvFieldError, MuxError,
};
