/*
 * send_synthetic.c — minimal C consumer of libtstrans.
 *
 * Build:
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        int rc = tst_mux_sender_send_video(s, nal, sizeof(nal), i * 33000, i == 0);
        if (rc != 0) {
        ...
        rc = tst_mux_sender_send_klv(s, klv, klv_len, i * 33000);
        if (rc != 0) {
