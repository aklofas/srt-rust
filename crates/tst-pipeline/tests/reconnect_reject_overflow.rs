...
//! `TransportError::Backpressure("gap buffer full")` to the caller when
...
/// surface `TransportError::Backpressure("gap buffer full")` rather than
...
fn reject_policy_surfaces_backpressure_when_gap_full() {
    ...
    assert!(
        matches!(first, TransportError::Broken(_)),
...
fn drop_oldest_policy_does_not_surface_backpressure_on_overflow() {
    ...
    let first = managed.send_bytes(b"first").unwrap_err();
    assert!(matches!(first, TransportError::Broken(_)));
    ...
    let second = managed.send_bytes(b"second").unwrap_err();
    assert!(
        matches!(second, TransportError::Broken(_)),
