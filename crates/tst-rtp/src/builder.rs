...
impl RtspServerBuilder {
    ...
    pub fn new(url: &str) -> Result<Self, RtspServerError> {
        ...
        // T2 sibling task is adding `RtspUrl::validate_for_server_bind`.
        // Until that lands, inline the IP-literal check; the merge agent
        // will swap this for `parsed.validate_for_server_bind()?` when
        // T2 is reconciled. TODO(T2-merge): switch to the method form.
        if parsed.host.parse::<std::net::IpAddr>().is_err() {
            return Err(RtspServerError::UrlParse(crate::url::UrlError::BadScheme {
                scheme: format!(
                    "server bind host must be an IP literal, got '{}'",
                    parsed.host
                ),
            }));
        }
