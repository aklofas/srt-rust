...
//! Run via `cargo run -p srt-core --example gen_h266_fixtures`.
//!
//! Fixtures land at `tests/fixtures/codec/h266/`. Tests load them and
//! assert the parsed fields. Hand-crafting is preferred over
//! reference-encoder output for unit tests because (a) the bytes are
//! pinned, (b) we exercise the smallest valid path, (c) no external
...
/// Same bytes as codec::h266::sps::tests::minimal_sps_rbsp() — captured
/// via dbg! in that test.
///
/// Extended in Task 4.2 to cover the full SPS body walk through
/// sps_vui_parameters_present_flag (all optional fields disabled).
