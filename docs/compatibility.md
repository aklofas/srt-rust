...
| `srtc_*_add_stream` (multi-stream binding follow-up) | ⏳ Planned | Today the C ABI exposes single-target `srtc_*_send_video` / `_send_klv` only and rejects with `SRTC_E_INVALID_USAGE` if the wrapped Rust `Muxer` holds more than one stream of that kind. The Rust core already supports multi-stream (≤16 video + ≤16 KLV); the C-side `srtc_stream_handle_t` + `_video_to` / `_klv_to` siblings are scheduled as a separate plan. See "Multi-stream `mpegts::mux` — `srt-c` binding surface" in `deferred-features.md`. |
