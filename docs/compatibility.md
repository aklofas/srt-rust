...
| Multi-program demux at the C ABI | ❌ Deferred | Rust demuxer fully supports multi-program; the C ABI demux event surface (`tst_demux_receiver_t`) does not yet exist — raw byte and TS-packet receiver surfaces ship in Phases 1 + 2. Multi-program demux event emission rides with Phase 3. |
