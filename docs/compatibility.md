...
| `srtc_*_add_stream` (multi-stream Path 3) | ⏳ Planned | Today: `add_video` / `add_klv` only. Path 3 lifts the v0 single-stream limit additively without ABI break. |
...
| Distribution artifacts | ✅ Full | `libsrtc.so` + `libsrtc.a` + `srtc.h` + `srtc.pc`. Tarball staged manually; GitHub Releases publishing not automated in v0. |
...
| Pre-emptive close cancellation while parked in libsrt | ⏳ Planned | v0 close blocks until any in-flight send returns; tightening to libsrt's "close-anywhere unblocks the parked send" idiom is a follow-up. |
