...
- **Recipe 9c: Publish MPEG-TS as an HLS stream** — [hls.md](sending/hls.md) — Built-in HTTP server serves `.ts` segments + `.m3u8` playlist; LIVE/EVENT/VOD modes; optional Basic auth + HTTPS via rustls. **Experimental, Rust-only — not in the v0.1.0 published wheels/JAR; see [deferred-features.md](/docs/project/deferred-features.md).**
