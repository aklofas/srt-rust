...
> **Who this is for:** You've read the guides and want to crib working code for a specific task. Each recipe is a self-contained Rust snippet (or example pointer) you can paste into your project.
...
- **Recipe 9c: Publish MPEG-TS as an HLS stream** — [hls.md](sending/hls.md) — Built-in HTTP server serves `.ts` segments + `.m3u8` playlist; LIVE/EVENT/VOD modes; optional Basic auth + HTTPS via rustls.
- **Recipe 9d: Send MPEG-TS over RIST** — [rist.md](sending/rist.md) — VideoLAN librist 0.2.10 bindings; ARQ-recovered UDP with Simple + Main profiles and AES-128/192/256 PSK encryption. Sender side.
