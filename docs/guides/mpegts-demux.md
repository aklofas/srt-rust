...
| `SamplePayload` | `Video { codec, raw, random_access_indicator, av1_carriage }`, `Audio { codec, frames }`, `Subtitle { codec, payload }`, `Unknown { stream_type, raw }`. **Raw-first:** `raw` is the exact encoded access unit (a `SharedBytes`); call `split_video(&raw, codec, av1_carriage)` to split it into a `VideoPayload` — `Nals(Vec<NalUnit>)` for H.264 / H.265 / H.266 or `Obus(Vec<Obu>)` for AV1. |
