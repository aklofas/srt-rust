...
```toml
[dependencies]
tst-core     = { version = "0.5.0", default-features = false }
tst-pipeline = { version = "0.5.0", default-features = false }
```
...
- **`std` is on by default.** A bare `tst-core = { version = "0.5.0" }` pulls in the standard library. Always add `default-features = false` for bare-metal targets — the compiler will reject the build if you forget (the `*-none-*` target has no `std`).
