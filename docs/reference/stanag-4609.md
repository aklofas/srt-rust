...
| KLV metadata sets | ST 0601.19 · ST 0102.12 · ST 0903.6 · ST 0605.10 | ✅ Supported | Typed encode/decode for all four sets; 142 of 143 ST 0601 items typed or structured (Item 66 deprecated stays unknown-passthrough by design). See [KLV guide](/docs/guides/klv.md). |
...
| Minimum Metadata Set compliance validator | MISB ST 0902.8 | ⚙️ Opt-in | `klv::st0601::validate_mismms` returns a `Vec<MismmsViolation>` identifying which required ST 0902 fields are absent or out of range. Rust, Python, and JVM (the C ABI carries raw KLV but exposes no typed KLV APIs). See [per-language snippets](#per-language-snippets) below. |
