...
#
# This is deliberately NOT named `check-*.sh`: the pre-push glob
# (`for s in scripts/check-*.sh`) runs every check, and stress is far too slow
...
#
# Usage:
#   scripts/stress-network-tests.sh                 # quick scope, 3 loops
#   STRESS_LOOPS=5 scripts/stress-network-tests.sh
#   STRESS_SCOPE=full scripts/stress-network-tests.sh
set -euo pipefail
